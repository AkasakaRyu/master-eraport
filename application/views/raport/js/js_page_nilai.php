...
<script>
	$(document).ready(function() {
		$("#dtTable").DataTable({
			"processing": true,
			"ajax": {
				"url": "<?= base_url('raport/nilai/list_data/') ?>",
				"type": "POST"
			}
		});
		$("#siswa_id").select2({
		...
		$('#FrmNilai').submit(function(e) {
			...
			swal({
				...
				if(Oke) {
					$.ajax({
						type: "POST",
						url: "<?= base_url('raport/nilai/simpan/') ?>",
						data: $("#FrmNilai").serialize(),
						timeout: 5000,
		...
		$(document).on('click','#edit',function() {
			jQuery.ajax({
				type: "POST",
				url: "<?= base_url('raport/nilai/get_data/') ?>",
				dataType: 'json',
				data: {
		...
		$(document).on('click','#hapus',function() {
			swal({
				...
				if(Oke) {
					$.ajax({
						type: "POST",
						url: "<?= base_url('raport/nilai/hapus/') ?>",
						data: {
							nilai_id: $(this).attr("data")
						},
						timeout: 5000,
